...
if st.button("Valider"):
    code = cmd.strip().upper().replace("[", "").replace("]", "")
    ...
    if code == "VEILLE":
        ...
        for c, data in focus:
            st.markdown(f"* [{c}] | {data['name']} [Sollicité : {data['count']} fois]")

        st.markdown("📋 **Autres sujets actifs :**")
        for c, data in others:
            st.markdown(f"* [{c}] | {data['name']} [Sollicité : {data['count']} fois]")

    elif code in st.session_state.subjects:
        st.session_state.subjects[code]["count"] += 1
        st.markdown(f"### 📰 Bulletin : {st.session_state.subjects[code]['name']}")
        st.info(f"Actualités pour le sujet {code} (Sollicité : {st.session_state.subjects[code]['count']} fois).")
    else:
...
st.markdown("**Sujet suivant** — Tape le `[Code]` d'un sujet pour générer son bulletin.")
