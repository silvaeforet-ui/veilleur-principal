...
# Force Streamlit à vider son ancienne mémoire pour éviter le bug de l'écran blanc
if "app_version" not in st.session_state or st.session_state.app_version != "v_finale":
    st.session_state.clear()
    st.session_state.app_version = "v_finale"
...
# Ajout dynamique des thématiques restantes pour atteindre les 21 sujets
CODES_RESTANTS = [
...
for code, nom in CODES_RESTANTS:
    REFERENTIEL_SUJETS.append({
        ...
        "news": [
            {
                "id": 1,
                "title": f"Dernières évolutions : {nom}",
                ...
                "deep": "L'analyse approfondie de cette thématique révèle de fortes résistances au changement liées à l'empilement des procédures. La clé de déblocage réside systématiquement dans la transversalité inter-services et la réappropriation du sens de l'action publique par les agents concernés.",
...
# Bloc Try/Except pour s'assurer qu'au pire des cas, on affiche l'erreur proprement
try:
    if not clean_cmd or clean_cmd == "VEILLE":
        ...
        if 0 <= news_idx < len(sub["news"]):
            ...
            st.error(f"Oups, je ne trouve pas l'actualité n°{num_part} dans le bulletin {sub['code']}.")
