...
# --- DISJONCTEUR DE CACHE ---
if "app_version" not in st.session_state or st.session_state.app_version != "v_finale_3":
    st.session_state.clear()
    st.session_state.app_version = "v_finale_3"

# --- RÉFÉRENTIEL DES SUJETS ---
REFERENTIEL_SUJETS = [
    {
        "code": "AP-DES",
...
# Ajout dynamique des thématiques restantes
CODES_RESTANTS = [
...
if not clean_cmd or clean_cmd == "VEILLE":
    ...
    for idx, sub in focus:
        st.markdown("* **[" + str(idx + 1) + "]** [" + sub['code'] + "] | " + sub['name'] + " *(Vu : " + str(sub['count']) + " fois)*")
    ...
    if 0 <= news_idx < len(sub["news"]):
        ...
        st.markdown("### 🔍 Approfondissement analytique — News n°" + str(item["id"]))
        st.markdown("**Sujet associé :** " + sub["name"] + " (`" + sub["code"] + "`)")
        st.info(item["deep"])
        st.markdown("📌 **Fondé sur l'analyse critique de :** " + str(item["source"]))
    else:
        st.error("Oups, je ne trouve pas l'actualité n°" + str(num_part) + " dans le bulletin " + sub["code"] + ".")
    ...
    if 0 <= news_idx < len(sub["news"]):
        item = sub["news"][news_idx]
        sub["count"] += 1
        st.markdown("### 💼 Piste de réflexion pour ton réseau — News n°" + str(item["id"]))
        st.success(item["linkedin"])
        st.ma
