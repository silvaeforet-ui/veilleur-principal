...
if "app_version" not in st.session_state or st.session_state.app_version != "v_finale_2":
    st.session_state.clear()
    st.session_state.app_version = "v_finale_2"
...
# Routage des commandes (sans bloc try/except pour éviter les erreurs d'indentation)
if not clean_cmd or clean_cmd == "VEILLE":
    ...
    if 0 <= news_idx < len(sub["news"]):
        item = sub["news"][news_idx]
        sub["count"] += 1
        st.markdown(f"### 💼 Piste de réflexion pour ton réseau — News n°{item['id']}")
