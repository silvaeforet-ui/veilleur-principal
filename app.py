...
def charger_donnees():
    """Charge les données de manière hermétique pour éviter les corruptions de cache."""
    data = [
        {
            "code": "AP-DES",
    ...
    # Ajout dynamique des thématiques restantes pour le menu complet
    codes_restants = [
    ...
    for code, nom in codes_restants:
        data.append({
            ...
            "news": [
                {
                    "id": 1,
                    "title": "Dernières évolutions : " + nom,
                    "summary": "Les pratiques territoriales autour de cette thématique connaissent une accélération marquée. Les collectivités s'organisent pour intégrer de nouveaux cadres normatifs tout en préservant l'agilité opérationnelle de leurs équipes de terrain.",
                    "source": "Synthèse documentaire interne (2026)",
                    "confidence": "Moyen (en cours de consolidation)",
                    "deep": "L'analyse approfondie révèle de fortes résistances au changement liées à l'empilement des procédures. La clé de déblocage réside systématiquement dans la transversalité inter-services et la réappropriation du sens de l'action publique par les agents concernés.",
                    "linkedin": "Le traitement de cette thématique nous oblige à repenser nos silos administratifs pour plus d'efficacité sur nos territoires ! 🚀 #Transition #ActionPublique"
                }
            ]
        })
...
# --- DISJONCTEUR DE CACHE ULTIME ---
# Si la version change, on rase tout et on recommence proprement
if "APP_VERSION_5" not in st.session_state:
    st.session_state.clear()
    st.session_state["APP_VERSION_5"] = True
...
# --- LE MOUCHARD : Le bloc try intercepte TOUTES les erreurs ---
try:
    if not clean_cmd or clean_cmd == "VEILLE":
        sorted_subs = sorted(
            enumerate(st.session_state.subjects), key=lambda x: x[1].get("count", 0)
        )
        focus = sorted_subs[:2]
        others = sorted_subs[2:]
        random.shuffle(others)

        st.markdown("🎯 **Ce que je te suggère aujourd'hui (Sujets peu explorés) :**")
        for idx, sub in focus:
            st.markdown("* **[" + str(idx + 1) + "]** [" + sub.get("code", "") + "] | " + sub.get("name", "") + " *(Vu : " + str(sub.get("count", 0)) + " fois)*")
        ...
        if 0 <= news_idx < len(news_list):
            ...
            st.markdown("### 🔍 Approfondissement analytique — News n°" + str(item.get("id", "")))
            st.markdown("**Sujet associé :** " + sub.get("name", "") + " (`" + sub.get("code", "") + "`)")
            st.info(item.get("deep", ""))
            st.markdown("📌 **Fondé sur l'analyse critique de :** " + str(item.g
