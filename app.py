...
REFERENTIEL_SUJETS = [
    ...
    },
    {
        ...
        "linkedin": "Le grand défi des métropoles ? Réussir à penser et à apaiser la ville à hauteur d'habitant, au cœur même d'un tissu urbain très dense et vivant. 🏙️ #Urbanisme #Proximite"
    },
    {
        "code": "CUL-CIN",
        "name": "Cinéma & Séries exigeants",
        "ideas": [
            "**🎞️ Décrypter notre monde** : Servons-nous des séries complexes pour mieux comprendre les dynamiques de pouvoir qui nous entourent.",
            "**🔬 Un super labo social** : Regardons la fiction comme un miroir de nos propres mutations et angoisses contemporaines.",
            "**🍿 Inspirer nos pratiques** : Et si on analysait une bonne scène de crise tirée d'un film pour nourrir nos réflexions managériales ?"
        ],
        "deep": "Regarder une œuvre exigeante n'est jamais une perte de temps. C'est un formidable outil pour prendre du recul, aiguiser notre esprit critique et décoder les récits institutionnels qui nous gouvernent.",
        "linkedin": "La fiction n'est pas qu'un simple divertissement du dimanche soir. C'est surtout un laboratoire critique passionnant où s'expérimentent nos futurs collectifs. 🎬🍿 #Cinema #EspritCritique"
    }
]
...
cmd = st.text_input(
    "Commande",
    placeholder="Tape un numéro, un [CODE], 1+, etc.",
...
# Affichage par défaut de la liste si aucune commande n'est saisie
if not clean_cmd or clean_cmd == "VEILLE":
    ...
    st.markdown("🎯 **Ce que je te suggère aujourd'hui (peu exploré récemment) :**")
    for idx, sub in focus:
        st.markdown(f"* **[{idx + 1}]** [{sub['code']}] | {sub['name']} [Vu : {sub['count']} fois]")

    st.markdown("📋 **Tes autres sujets sur le feu :**")
    ...
    if 0 <= idx < len(st.session_state.subjects):
        ...
        st.markdown(f"### 🔍 On creuse le sujet : {sub['name']} (`{sub['code']}`)")
        st.info(sub["deep"])
    else:
        st.error("Mince, je n'ai pas trouvé ce numéro d'actualité. On réessaie ?")
    ...
    sub["count"] += 1
    st.markdown(f"### 💼 Une idée pour ton prochain post LinkedIn ({sub['code']})")
    st.success(sub["linkedin"])
    ...
    if selected_sub:
        ...
        st.markdown(f"## 💡 Quelques pistes pour toi : {selected_sub['name']} (`{selected_sub['code']}`)")
        st.markdown(f"*Tu as consulté ce sujet {selected_sub['count']} fois.*")
        st.markdown("---")
        for idea in selected_sub["ideas"]:
            st.markdown(f"* {idea}")
    else:
        st.error("Mince, je n'ai pas reconnu ce code ou ce numéro. Tu peux taper un chiffre valide, un code (ex: TEC-IAG), ou '1+'.")

st.markdown("---")
st.markdown("## 🎛️ Ta boîte à outils")
st.markdown("**Sujet suivant** — Tape simplement le numéro ou le `[Code]` d'un sujet.")
st.markdown("**Creuser une info** — Ajoute un '+' après le numéro (ex: `1+`).")
st.markdown("**Préparer un post LinkedIn** — Tape `[N°]linkedin` (ex: `1linkedin`).")
st.markdown("**Revenir à la liste principale** — Efface juste le champ de commande ci-dessus.")
